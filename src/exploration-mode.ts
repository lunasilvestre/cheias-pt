...
import { enableInteraction, disableInteraction, getMap } from './map-setup';
import { setLayerOpacity } from './layer-manager';

let explorationActive = false;
let togglesWired = false;

/**
 * Maps toggle data-layer attributes to actual MapLibre layer IDs
 * and their default explore-mode opacity.
 */
const TOGGLE_LAYER_MAP: Record<string, { layers: string[]; opacity: number }> = {
 ...
 */
export function geolocateUser(): void {
  const map = getMap();
 ...
 */
export function isExplorationActive(): boolean {
...
function wireToggles(): void {
  ...
  panel.addEventListener('change', (e) => {
    ...
    const checkbox = target.closest('input[data-layer]') as HTMLInputElement | null;
    if (!checkbox) return;
 ...
 */
function applyAllToggles(): void {
  const map = getMap();
  if (!map) return;

  const checkboxes = document.querySelectorAll<HTMLInputElement>('#exploration-controls input[data-layer]');
